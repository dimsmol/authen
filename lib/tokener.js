...
Tokener.prototype.applyRenewal = function (res, renewal) {
	...
	res.setHeader(names.nameIssued, JSON.stringify(renewalResult.issued));
...
Tokener.prototype.getAuthData = function (req) {
	var result = null;
	var tokenFromCookie = false;
	var token = req.getHeader(this.options.headers.name);
	if (token == null && !this.options.cookies.useLimited && req.cookies != null) {
		token = req.cookies[this.options.cookies.name];
	...
	if (token != null) {
		...
		if (!tokenFromCookie && req.cookies != null) {
			result.additionalToken = req.cookies[this.options.cookies.name];
		}
	}
	return result;
};
...
Tokener.prototype.createAuthResult = function (token, renewalTokenInfo) {
	...
	if (renewalTokenInfo != null) {
		result.renewal = {
			result: this.createTokenResult(renewalTokenInfo),
...
Tokener.prototype.getLastRevokationTime = function (token) {
...
Tokener.prototype.isRevoked = function (token) {
	var lastRevokationTime = this.getLastRevocationTime(token);
	return lastRevokationTime != null &&
		(token.time <= lastRevokationTime ||
			// do not trust automatic renewals for a while to ensure
			// renewal was issued after all app instances was surely informed about revokation
			!token.isStrong && token.time <= lastRevokationTime + this.options.postRevokationTrustDelay);
};
...
Tokener.prototype.renewCookieEnabled = function (res, token) {
	var result = this.renew(token);
...
Tokener.prototype.createTokenInternal = function (data) {
	return {
		data: data,
		token: this.signer.sign(this.pack(data))
	};
};
...
Tokener.prototype.createTokenResult = function (tokenInfo, opt_skipToken) {
...
Tokener.prototype.setCookies = function (res, tokenInfo) {
	...
	var cookieOptions = this.options.cookies;
	var expires = (tokenInfo.isSessionLifetime ? null : new Date(tokenInfo.data.time + this.options.timeout));
...
Tokener.prototype.parseTokenInternal = function (token) {
	...
	if (token) {
		...
		if (buffer) {
			...
			if (data && data.time + this.options.timeout >= Date.now()) {
				result = data;
			}
		}
	}
...
Tokener.prototype.getExpectedIdentity = function (req) {
	var result = null;
	var expectedIdentityStr = req.getHeader(this.options.headers.nameExpected);
