"use strict";
var Tokener = function (signer, opt_options) {
	this.signer = signer;
	this.prefix = (opt_options != null ? opt_options.prefix : ''); // NOTE cannot contain separator
...
Tokener.prototype.extractTokenData = function (token, additionalToken, cb) {
	...
	if (tokenData != null && tokenData.isLimited) {
		...
		if (tokenData.issued != limitedTokenData.issued || tokenData.identityStr != limitedTokenData.identityStr) {
			tokenData = null;
		}
	}
	cb(null, tokenData);
...
Tokener.prototype.extractSingleTokenData = function (token) {
	...
	if (token != null) {
		...
		if (this.signer.isValidSignature(parts.signature, parts.tokenDataStr, parts.algoName, parts.key)) {
...
Tokener.prototype.encode = function (tokenData) {
	var tokenDataStr = this.stringify(tokenData);
	var signatureInfo = this.signer.calcSignature(tokenDataStr);
	return this.packToToken(tokenDataStr, signatureInfo);
};
...
Tokener.prototype.unpackToken = function (token) {
	return this.extractParts(token, [
		'prefix', 'algoName', 'key', 'signature', 'tokenDataStr'
	], this.separator);
};
...
Tokener.prototype.parse = function (tokenDataStr) {
	...
	if (!result.identityStr || isNaN(result.issued)) {
		result = null;
	}
	else {
...
Tokener.prototype.extractParts = function (str, names, separator) {
	...
	for (var i = 0; i <= lastNameIdx; i++) {
		var name = names[i];
		var idx = (i < lastNameIdx ? str.indexOf(separator, pos) : undefined);
