...
Tokener.prototype.parseTokenToPacked = function (token) {
	...
	if (data) {
		...
		if (this.signer != null) {
			...
			if (info != null) {
				var isOk = this.isValidSignature(info.signature, info.data, info.algo, info.key);
...
Tokener.prototype.unpackData = function (data) {
	var result = this.unpack(data, '', ['issued', 'data']);
...
Tokener.prototype.packCryptInfo = function (info) {
	return this.pack(['c', info.algo, info.key, info.data]);
};

Tokener.prototype.unpackCryptInfo = function (data) {
	return this.unpack(data, 'c', ['algo', 'key', 'data']);
};

// sign

Tokener.prototype.packSignInfo = function (info) {
	return this.pack(['s', info.algo, info.key, info.signature, info.data]);
};

Tokener.prototype.unpackSignInfo = function (data) {
	return this.unpack(data, 's', ['algo', 'key', 'signature', 'data']);
};
