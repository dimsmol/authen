...
AuthProvider.prototype.login = function (res, identityStr, options, cb) {
	...
	this.tokener.createToken(identityStr, {
		...
		if (res != null) {
			this.adapter.applyAuthData(res, tokenInfo, this.getMaxAge(isSessionLifetime), useCookies);
		}
		cb(null, {
			tokenInfo: tokenInfo,
			result: this.prepareTokenResult(tokenInfo)
		});
	}, cb));
};
...
AuthProvider.prototype.authByData = function (authData, res, options, cb) {
	...
	if (authData == null) {
		...
		this.tokener.extractTokenData(authData.token, authData.additionalToken, errh(function (tokenData) {
			...
			if (tokenData == null) {
				...
				self.checkRevoked(tokenData, errh(function (isRevoked) {
					if (isRevoked) {
						...
						self.tokener.renewToken(tokenData, self.options.useLimitedToken, errh(function (renewalTokenInfo) {
							if (res != null) {
								self.adapter.applyRenewal(res, renewalTokenInfo, self.getMaxAge(tokenData.isSessionLifetime), tokenData.useCookies);
							}
							result.renewalTokenInfo = renewalTokenInfo;
							result.renewal = self.prepareTokenResult(renewalTokenInfo, tokenData.isSessionLifetime);
							cb(null, result);
						}, cb));
					}
					else {
...
AuthProvider.prototype.isValidIssued = function (tokenData) {
	...
	return tokenData.issued - this.options.allowedIssuedClockDeviation <= Date.now();
};

AuthProvider.prototype.isExpired = function (tokenData) {
	// tokenData: as encoded to token by tokener
	return tokenData.issued + this.options.maxAge < Date.now();
};
