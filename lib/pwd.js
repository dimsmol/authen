...
Pwd.prototype.hash = function (pwd, cb) {
	var self = this;
	this.hashInternal(pwd, errh(function (hash, salt) {
		cb(null, [
			self.algoName,
...
Pwd.prototype.verifyInternal = function (pwd, hash, salt, iterations, len, cb) {
	var self = this;
	crypto.pbkdf2(pwd, salt, iterations, len, errh(function(calculatedHash){
		cb(null, constantTimeEquals(hash, new Buffer(calculatedHash, 'binary')));
	}, cb));
};

Pwd.prototype.createSalt = function (cb) {
	crypto.randomBytes(this.options.saltLen, errh(function (salt) {
